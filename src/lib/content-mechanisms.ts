    ...
    variations: [
      {
        value: "value",
        label: "Value",
        hint: 'Tracking system reveal · "Copy this exact system" · "The exact system of X that let Y do Z."',
      },
      {
        value: "problem_solution",
        label: "Problem-Solution",
        hint: "ONE niche problem → 1-3 sharp solutions. Never 30 solutions.",
      },
    ],
  },
  credibility: {
    ...
    variations: [
      {
        value: "testimonial",
        label: "Testimonial",
        hint: "Proof you CAN help — client's situation in depth, every point labelled as a problem we solve.",
      },
      {
        value: "case_study",
        label: "Case-Study",
        hint: 'Proof of HOW you help — step-by-step so the viewer thinks "I could do this too."',
      },
    ],
  },
  authoritative: {
    ...
    variations: [
      {
        value: "attention_lifestyle",
        label: "Attention-Driven (Lifestyle)",
        hint: "Premium look that raises status — always tied back to the skill/mechanism.",
      },
      {
        value: "industry_leader",
        label: "Industry Leader",
        hint: '"POV: meeting your X in Y after doing Z" · "Building an X from scratch."',
      },
    ],
  },
  relatability: {
    ...
    variations: [
      {
        value: "storytelling",
        label: "Storytelling",
        hint: '"How I went from X to Z" — pull them into your life so they visualise their own.',
      },
      {
        value: "personality",
        label: "Personality",
        hint: "How hard you work, the life you wanted, going back to your hometown — align with ICP values.",
      },
    ],
  },
};
...
/** One fixed color per mechanism, reused everywhere a mechanism shows up
 * (Traffic's content leaves, Content Signals' mix/drivers/evidence) so the
 * same mechanism always reads as the same color across the whole app —
 * never a second, locally-invented color per surface. Picked from the
 * existing spectrum/semantic tokens (styles.css); deliberately avoids
 * --spectrum-hot, which already means "cash/revenue" everywhere else. */
export const MECHANISM_COLORS: Record<MechanismKey, string> = {
  educational: "var(--spectrum-cold)",
  credibility: "var(--color-success)",
  authoritative: "var(--spectrum-mid)",
  relatability: "var(--color-warning)",
};

export function variationLabel(
  mech: string | null | undefined,
...
export const VARIATION_QUESTIONS: Record<string, VariationQuestion[]> = {
  ...
  value: [
    {
      key: "system_name",
      label: "What's the system / framework being revealed?",
      type: "text",
      placeholder: "e.g. The 3-touch DM tracking sheet",
    },
    {
      key: "who_got_result",
      label: "Who used it and what exact result did they get?",
      type: "text",
      placeholder: "e.g. Jayden — 4 calls booked in 6 days",
    },
    {
      key: "steps",
      label: "The 1-3 steps of the system (in order)",
      type: "textarea",
      placeholder: "1. …\n2. …\n3. …",
    },
    {
      key: "giveaway_depth",
      label: "How much do we give away?",
      type: "select",
      options: [
        "Full system (max trust)",
        "Steps but not the exact scripts",
        "Overview only (tease the how)",
      ],
    },
  ],
  problem_solution: [
    {
      key: "one_problem",
      label: "The ONE niche problem (nothing broader)",
      type: "text",
      placeholder: "e.g. Leads reply then ghost after the price",
    },
    { key: "why_it_happens", label: "Why it actually happens (the real cause)", type: "textarea" },
    {
      key: "solutions",
      label: "1-3 sharp solutions — no more",
      type: "textarea",
      placeholder: "1. …\n2. …",
    },
  ...
  testimonial: [
    ...
    {
      key: "before_state",
      label: "Their BEFORE in detail (numbers, feelings, situation)",
      type: "textarea",
    },
    { key: "after_state", label: "Their AFTER (hard numbers + timeframe)", type: "textarea" },
    {
      key: "problems_solved",
      label: "Every problem we solved (each becomes a labelled beat)",
      type: "textarea",
      placeholder: "no offer clarity, no tracking, low show rate…",
    },
    {
      key: "proof_asset",
      label: "Proof asset we can show on screen",
      type: "select",
      options: [
        "Screen recording of their dashboard",
        "Payment / Stripe screenshot",
        "Voice note or clip of the client",
        "DM screenshots",
        "None — spoken only",
      ],
    },
  ...
  case_study: [
    {
      ...
      type: "text",
      placeholder: "e.g. $18k month from 5 reels/week",
    },
    { key: "starting_point", label: "Starting point (so it feels replicable)", type: "textarea" },
    {
      key: "steps_taken",
      label: "Step-by-step of exactly what we did",
      type: "textarea",
    ...
    {
      key: "replicability",
      label: "How replicable should it feel?",
      type: "select",
      options: [
        '"You could do this today"',
        '"You could do this with help"',
        '"This is why you need the system"',
      ],
    },
  ],
  // Authoritative
  attention_lifestyle: [
    {
      key: "visual_hook",
      label: "The premium visual that stops the scroll",
      type: "text",
      placeholder: "e.g. Balcony setup in Dubai, laptop open on the tracker",
    },
    {
      key: "status_signal",
      label: "What status signal are we showing?",
      type: "select",
      options: [
        "Location / travel",
        "Environment / office",
    ...
    {
      key: "tie_back",
      label: "How does it tie back to the skill / mechanism? (non-negotiable)",
      type: "textarea",
      placeholder: "This exists because of X system, not luck",
    },
    {
      key: "voiceover",
      label: "Voiceover or on-screen text only?",
      type: "select",
      options: ["Voiceover", "On-screen text only", "Both"],
    },
  ],
  industry_leader: [
    {
      key: "pov_line",
      label: "The POV / positioning line",
      type: "text",
      placeholder: "POV: meeting your mentor after 12 months of daily reels",
    },
    {
      key: "authority_proof",
      label: "What proves you lead the space?",
      type: "textarea",
      placeholder: "clients, volume, results, rooms you're in",
    },
  ...
  storytelling: [
    {
      key: "from_to",
      label: "From X → to Z (the arc)",
      type: "text",
      placeholder: "e.g. Broke in a basement → $40k/mo at 19",
    },
    ...
    { key: "lesson", label: "The lesson the viewer should steal", type: "text" },
    {
      ...
      label: "Arc length",
      type: "select",
      options: ["Single reel (30-45s)", "Long-form (90s+)", "Multi-part sequence"],
    },
  ],
  personality: [
    {
      key: "trait",
      label: "Which trait are we showing?",
      type: "select",
      options: [
        "Work ethic / obsession",
        "Loyalty to where you're from",
        "Discipline / routine",
        "Humour / unfiltered takes",
        "Family / why you started",
      ],
    },
    {
      key: "scene",
      label: "The real scene / moment we're filming",
      type: "textarea",
      placeholder: "e.g. 4:50am gym before calls, hometown drive",
    },
    {
      key: "icp_value",
      label: "Which ICP value does this align with?",
      type: "text",
      placeholder: "e.g. they want to prove people wrong",
    },
    {
      key: "vulnerability",
      label: "How raw do we go?",
      type: "select",
      options: ["Very raw / unfiltered", "Honest but composed", "Light and funny"],
    },
  ],
};
...
const KEYWORDS: Record<MechanismKey, RegExp> = {
  credibility:
    /(scam|trust|real|legit|fake|proof|believe|guru|burn|ripped off|refund|guarantee|testimonial|results? (are|were)? ?real|does it (actually )?work|risk)/i,
  educational:
    /(how|didn'?t know|confus|unclear|understand|learn|system|process|strategy|steps|taught|explain|knowledge|value|tactic)/i,
  authoritative:
    /(best|top|authority|leader|premium|expensive|price|worth it|cheaper|competitor|why you|status|level|serious)/i,
  relatability:
    /(relat|like me|my situation|same as|story|felt|struggl|alone|young|age|from where|background|understand me|personality|vibe)/i,
};
...
export function reelSplit(mix: Record<MechanismKey, number>, totalReels: number) {
  return MECHANISM_KEYS.map((k) => ({
    mechanism: k,
    reels: Math.max(1, Math.round((mix[k] / 100) * totalReels)),
  }));
}
