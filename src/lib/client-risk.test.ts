...
import { clientAtRiskReason, daysUntilDate } from "./client-risk";
...
describe("clientAtRiskReason", () => {
  it("is not at-risk when there's no renewal date", () => {
    expect(clientAtRiskReason({ renewal_date: null, renewal_conv_started: false }, 30, FIXED_NOW)).toBeNull();
  });

  it("flags an overdue renewal regardless of conversation status", () => {
    const reason = clientAtRiskReason({ renewal_date: "2026-08-01", renewal_conv_started: true }, 30, FIXED_NOW);
    expect(reason).toBe("renewal 12d overdue");
  });

  it("flags a renewal inside the at-risk window with no conversation started", () => {
    const reason = clientAtRiskReason({ renewal_date: "2026-08-20", renewal_conv_started: false }, 30, FIXED_NOW);
    expect(reason).toBe("renewal in 7d, no convo");
  });

  it("does NOT flag a renewal inside the window if a conversation already started", () => {
    expect(clientAtRiskReason({ renewal_date: "2026-08-20", renewal_conv_started: true }, 30, FIXED_NOW)).toBeNull();
  });

  it("does NOT flag a renewal outside the configured window", () => {
    // 60 days out, window is 30
    expect(clientAtRiskReason({ renewal_date: "2026-10-12", renewal_conv_started: false }, 30, FIXED_NOW)).toBeNull();
  });
