...
import {
  ...
  type ContentEngineSettingsT,
} from "./workspace-settings.functions";
import {
  normalizeFaqSignal,
...
export async function computeDemand(
  ...
  const [faqRes, settersRes, intakesRes, reelsRes] = await Promise.all([
    ...
    sb
      .from("content_pieces")
      .select(
        "id, mechanism, variation, platform, posted_at, pipeline_status, content_metrics(leads_generated, cash_collected_cents, hook_retention_pct, engagement_rate_pct, drop_off_rate_pct, views)",
      )
      .eq("org_id", orgId)
      .gte("created_at", fromIso)
      .lte("created_at", toIso)
  ...
  for (const s of (setters ?? []) as any[]) {
    const normalized = normalizeSettingCallSignal(s);
    if (normalized) signals.push(normalized);
    const text = [...(s.limiting_beliefs ?? []), ...(s.objections ?? []), s.ai_summary, s.notes]
      .filter(Boolean)
      .join(" · ");
    if (s.mechanism) {
  ...
  for (const i of (intakes ?? []) as any[]) {
    const normalized = normalizeOnboardingSignal(i);
    if (normalized) signals.push(normalized);
    const r = (i.responses ?? {}) as Record<string, string>;
    const text = [
      r.first_touch,
      r.first_touchpoint,
      r.discovery_touchpoint,
      r.decision_moment,
      r.pivotal_moment,
      r.beliefs_shifted,
      r.content_type_helped,
      r.join_earlier,
      r.join_sooner,
      r.hesitations,
      r.fear,
    ...
    const sc = hasStored
      ? addWeights(emptyWeights(), stored as MechanismWeights)
      : scoreText(text, 2);
    weights = addWeights(weights, sc);
    const top = pickTop(sc);
    if (top)
  ...
  const reelBuckets: Bucket[] = reelsWithBucket.map((p) => ({
...
export async function computeWeeklyContentCheck(
  ...
  if (ids.length) {
    const ms = unwrap<any[]>(
      await sb
        ...
        .select(
          "content_id, views, dms_generated, calls_booked, cash_collected_cents, leads_generated, engagement_rate_pct, drop_off_rate_pct, hook_retention_pct",
        )
        .in("content_id", ids),
      "Content metrics",
    );
  ...
  const reels = pieces.filter((p) =>
    ["reel", "tiktok", "youtube_short"].includes(p.platform as string),
  ).length;
  ...
  const worst =
    MECHANISM_KEYS.filter((k) => per[k].count > 0).sort((a, b) => score(a) - score(b))[0] ?? null;
    ...
    if (per[worst].withMetrics === 0) {
      ...
      for (const [platform, platPieces] of byPlatform) {
        ...
        for (const p of platPieces) {
          ...
          verdicts.push({
            platform,
            verdict: classifyPerformance(toPieceMetrics(m), baseline, config.minBucketSample),
          });
        }
      }
...
export async function analyzeContentSystem(
  ...
  const [
    ...
    sb
      .from("content_metrics")
      .select(
        "content_id, views, reach, shares, saves, likes, comments, dms_generated, calls_booked, closes, cash_collected_cents, leads_generated, avg_watch_pct, hook_retention_pct, three_sec_hold_pct, drop_off_rate_pct, engagement_rate_pct, follower_views, non_follower_views, followers_gained",
      )
      .eq("org_id", orgId)
    ...
    sb
      ...
      .select(
        "video_name, total_plays, unique_viewers, play_rate, avg_percent_watched, page_loads, captured_at",
      )
      .eq("org_id", orgId)
      .gte("captured_at", fromIso)
      .lte("captured_at", toIso)
    ...
    sb
      .from("faq_videos")
      .select("title, question, mechanism, clicks, plays, avg_watch_pct")
      .eq("org_id", orgId)
      .limit(60),
    sb
      .from("setter_call_signals")
      .select("setter_name, call_date, source, limiting_beliefs, objections, ai_summary")
      .eq("org_id", orgId)
      .gte("call_date", range.from)
      .lte("call_date", range.to)
      .limit(120),
    sb
      .from("calls")
      .select("status, showed, closed, cash_collected_cents, scheduled_for")
      .eq("org_id", orgId)
      .gte("created_at", fromIso)
      .lte("created_at", toIso)
      .limit(500),
  ...
  for (const p of (pieces ?? []) as any[]) {
    ...
    bucketsInWindow.set(bucketKey({ mechanism: p.mechanism, platform: p.platform }), {
      mechanism: p.mechanism,
      platform: p.platform,
    });
  }
  const bucketBaselines = await fetchBaselines(
    sb,
    orgId,
    Array.from(bucketsInWindow.values()),
    config.baselineWindowSize,
  );
  const bucketReads: {
    bucket: Bucket;
  ...
  for (const [key, bucket] of bucketsInWindow) {
    ...
    if (!baseline) continue;
    const piecesInBucket = (pieces ?? []).filter(
      (p: any) => p.mechanism === bucket.mechanism && p.platform === bucket.platform,
    );
    let strong = 0,
      typical = 0,
      underperforming = 0,
  ...
  const onboardingTags: Record<MechanismKey, number> = {
    educational: 0,
    credibility: 0,
    authoritative: 0,
    relatability: 0,
  ...
  for (const i of (intakes ?? []) as any[]) {
    ...
    let top: MechanismKey | null = null,
      topVal = 0;
    for (const k of MECHANISM_KEYS) {
    ...
    if (top) onboardingTags[top] += 1;
    else onboardingUntagged += 1;
  }

  const cash =
    ((payments ?? []) as any[]).reduce((s, p) => s + Number(p.amount_cents ?? 0), 0) / 100;
...
${
  demand.drivers
    .slice(0, 12)
    .map((d) => `- [${d.mechanism}] ${d.source}: ${d.detail} (weight ${d.weight})`)
    .join("\n") || "- none"
}

CONTENT PERFORMANCE BY MECHANISM (reel-level detail, not aggregate):
${
  Object.entries(perMech)
    .map(([k, v]) => {
...
${
  ((vslSnaps ?? []) as any[])
    ...
    .map(
      (v) =>
        `- ${v.video_name ?? "VSL"}: play rate ${v.play_rate}%, avg watched ${v.avg_percent_watched}%, plays ${v.total_plays}, page loads ${v.page_loads}`,
    )
    .join("\n") || "- none"
}
...
${
  ((setters ?? []) as any[])
    ...
    .map(
      (s) =>
        `- ${s.call_date} ${s.setter_name} [${s.source ?? "manual"}]: beliefs[${(s.limiting_beliefs ?? []).join("; ")}] objections[${(s.objections ?? []).join("; ")}]`,
    )
    .join("\n") || "- none"
}
...
{"limiting_beliefs":["..."],"objections":["..."],"mechanism":"educational|credibility|authoritative|relatability","summary":"2-3 sentences on what content would kill these objections"}`;
  ...
  try {
    ...
    return {
      limiting_beliefs: Array.isArray(json.limiting_beliefs)
        ? json.limiting_beliefs.slice(0, 12).map(String)
        : [],
