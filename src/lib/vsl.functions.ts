...
export type VslKind = "main" | "webinar" | "post_booking";
...
export const listVsls = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase.from("vsls").select("*").eq("org_id", org_id).order("created_at", { ascending: true });
...
export const upsertVsl = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: {
    id?: string; kind: VslKind; name: string; wistia_video_id?: string; sheet_url?: string;
    script?: string; transcript_json?: Array<{ t: number; text: string }>; notes?: string;
  }) => input)
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const org_id = await getOrgId(supabase, userId);
    const row: any = {
      org_id, kind: data.kind, name: data.name,
...
export const addSnapshot = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: {
    vsl_id: string; video_name?: string; total_plays?: number; unique_viewers?: number;
    play_rate?: number; avg_percent_watched?: number; page_loads?: number;
    engagement_json?: Array<{ sec: number; pct: number }>; source?: string;
  }) => input)
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const org_id = await getOrgId(supabase, userId);
    const { data: saved, error } = await (supabase as any).from("vsl_metric_snapshots").insert({
      vsl_id: data.vsl_id, org_id,
...
export const analyzeVsl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: vsl } = await (supabase as any).from("vsls").select("*").eq("id", data.vsl_id).maybeSingle();
    if (!vsl) throw new Error("VSL not found");
    const { data: snaps } = await (supabase as any).from("vsl_metric_snapshots")
      .select("*").eq("vsl_id", data.vsl_id).order("captured_at", { ascending: false }).limit(20);
    ...
    const trend = (k: string) => {
      const a = Number((first as any)[k] ?? 0), b = Number((latest as any)[k] ?? 0);
    ...
    const transcriptBlock = transcript.length
      ? transcript.map((l: any) => `[${Math.floor(l.t / 60)}:${String(Math.floor(l.t % 60)).padStart(2, "0")}] ${l.text}`).join("\n")
...
export const transcribeAudio = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { vsl_id: string; filename: string; mime: string; base64: string }) => input)
  .handler(async ({ data, context }) => {
    const apiKey = process.env.LOVABLE_API_KEY;
    if (!apiKey) throw new Error("LOVABLE_API_KEY not configured");
    ...
    const { error } = await (context.supabase as any).from("vsls")
