...
export function LiveTicker() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const setterCash = (setterAgg.data ?? []).reduce(
        (s, r) => s + (r.cash_collected_cents ?? 0),
        0,
      );
      const cash = Math.max(paymentsCash, callsCash + setterCash);
      const setterBooked = (setterAgg.data ?? []).reduce(
        (s, r) => s + (r.calls_on_calendar ?? 0),
        0,
      );
