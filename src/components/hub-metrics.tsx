    ...
    queryFn: async () => {
      ...
      const [acts, vsls, leads, calls, wins, clientsRes] = await Promise.all([
        supabase
          ...
          .select(
            "activity_date, role, dials, connections, links_sent, leads_contacted, qualified_convos, sets, calls_on_calendar, live_calls, closes",
          )
          .eq("org_id", orgId!)
        ...
        supabase
          ...
          .select(
            "captured_at, page_loads, total_plays, unique_viewers, play_rate, avg_percent_watched",
          )
          .eq("org_id", orgId!)
        ...
        supabase
          .from("leads")
          .select(
            "id, created_at, application_data, intent_score, priority, pipeline_stage, precall_video_watched",
          )
          .eq("org_id", orgId!)
      ...
      const engagementVals = vslList
        .map((v) => Number(v.avg_percent_watched ?? 0))
        .filter((n) => n > 0);
      ...
      const apps = leadList.filter(
        (l) => l.application_data && Object.keys(l.application_data as object).length > 0,
      );
      ...
      const quality = apps.length
        ? apps.reduce((s, l) => {
            ...
            const d = (l.application_data ?? {}) as Record<string, unknown>;
            const answered = Object.values(d).filter(
      ...
      const dialSpark = dayKeys.map((d) =>
        actList.filter((a) => a.activity_date === d).reduce((s, a) => s + (a.dials ?? 0), 0),
      );
      ...
      <div className="flex items-center gap-3 rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium text-destructive">
            Couldn't load operating metrics
          </div>
  ...
  return (
    <div className="space-y-3.5 animate-in fade-in-0 duration-300">
        ...
        tiles={[
          {
            ...
            value: fmt(d?.visits ?? 0),
            spark: d?.visitSpark,
            spectrum: "cold",
          },
          { label: "VSL plays", value: fmt(d?.plays ?? 0), spark: d?.playSpark, spectrum: "cold" },
          { label: "Play rate", value: rate(d?.plays ?? 0, d?.visits ?? 0), spectrum: "mid" },
          {
            label: "Engagement rate",
            value: d?.engagement ? `${d.engagement.toFixed(1)}%` : "—",
            tone: (d?.engagement ?? 0) >= 50 ? "success" : "warning",
            spectrum: "mid",
          },
          { label: "Unique viewers", value: fmt(d?.viewers ?? 0), spectrum: "cold" },
        ]}
      />
      ...
      <Band
        ...
        tiles={[
          ...
          },
          {
            ...
            value: d?.completion ? `${(d.completion * 100).toFixed(0)}%` : "—",
            tone: (d?.completion ?? 0) >= 0.8 ? "success" : "warning",
            spectrum: "mid",
          },
          {
            label: "Application quality",
            value: d?.quality ? `${d.quality.toFixed(1)}/5` : "—",
            tone: (d?.quality ?? 0) >= 3.5 ? "success" : "warning",
            spectrum: "mid",
          },
          {
            label: "Applications → booked call rate",
            value: rate(d?.booked ?? 0, d?.apps ?? 0),
            spectrum: "mid",
          },
          { label: "Pre-call vids watched", value: fmt(d?.precallWatched ?? 0), spectrum: "mid" },
        ]}
      />
      ...
      <Band
        ...
        to="/dm-setter"
        tiles={[
          { label: "Dials", value: fmt(d?.dials ?? 0), spark: d?.dialSpark, spectrum: "cold" },
          {
            label: "Pick-up rate",
            value: rate(d?.connections ?? 0, d?.dials ?? 0),
            tone: (d?.connections ?? 0) / Math.max(1, d?.dials ?? 1) >= 0.2 ? "success" : "warning",
            spectrum: "cold",
          },
          {
            label: "Qualified convo rate",
            value: rate(d?.qualified ?? 0, d?.contacted ?? 0),
            spectrum: "mid",
          },
          {
            label: "Show rate",
            value: rate(d?.showed ?? 0, d?.booked ?? 0),
            tone: (d?.showed ?? 0) / Math.max(1, d?.booked ?? 1) >= 0.6 ? "success" : "warning",
            spectrum: "mid",
          },
          {
            label: "Close rate (on show)",
            value: rate(d?.closed ?? 0, d?.showed ?? 0),
            tone: "success",
            spectrum: "hot",
          },
          { label: "Links sent", value: fmt(d?.linksSent ?? 0), spectrum: "cold" },
        ]}
      />
      ...
      <Band
        ...
        to="/fulfillment"
        tiles={[
          { label: "Active clients", value: fmt(d?.activeClients ?? 0), spectrum: "mid" },
          { label: "Daily W logs", value: fmt(d?.wins ?? 0), spectrum: "mid" },
          {
            label: "Financial wins",
            value: fmt(d?.financialWins ?? 0),
            tone: "success",
            spectrum: "hot",
          },
          {
            label: "Student cash logged",
            value: "$" + fmt((d?.winCash ?? 0) / 100),
            tone: "success",
            spectrum: "hot",
          },
          {
            label: "Avg student energy",
            value: d?.avgEnergy ? `${d.avgEnergy.toFixed(1)}/10` : "—",
            tone: (d?.avgEnergy ?? 0) < 5 && (d?.avgEnergy ?? 0) > 0 ? "danger" : "default",
          },
        ]}
      />
    </div>
    ...
    <section className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
      ...
      <div className="grid grid-cols-2 divide-y divide-border md:grid-cols-3 md:divide-y-0 md:divide-x xl:grid-cols-6">
        {tiles.map((t) => (
          <div
            key={t.label}
            className="group relative min-h-[88px] p-3.5 transition-colors hover:bg-muted/[0.06]"
          >
            <div className="text-3xs uppercase leading-snug tracking-[0.14em] text-muted-foreground">
              {t.label}
            </div>
            <div
              className={cn(
                "mt-2 font-mono text-xl font-semibold tabular-nums md:text-2xl",
              ...
              style={t.spectrum ? { color: SPECTRUM_VAR[t.spectrum] } : undefined}
            >
              {t.value}
            </div>
