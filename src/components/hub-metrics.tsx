...
function Band({
  ...
  return (
    <section className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
      <div className="flex items-center justify-between border-b border-border bg-background/20 px-4 py-2.5">
        <div className="flex items-center gap-2 text-2xs font-semibold uppercase tracking-wider">
          {icon}
          {title}
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 divide-y divide-border md:grid-cols-3 md:divide-y-0 md:divide-x xl:grid-cols-6">
        {tiles.map((t) => (
          <div key={t.label} className="p-3.5">
            <div className="text-3xs uppercase tracking-[0.14em] text-muted-foreground">
              {t.label}
            </div>
            <div
              className={cn(
                "mt-1.5 font-sans text-xl font-semibold tabular-nums md:text-2xl",
