        ...
        return (
          ...
          >
            <div className="px-3 py-2 border-b border-border flex items-center justify-between">
              <span
                className={cn(
                  "text-3xs uppercase tracking-wider rounded px-1.5 py-0.5",
                  CHIP_TONE_CLASSES[col.tone ?? "default"],
                )}
              >
                {col.label}
              </span>
            ...
            <div className="p-2 space-y-2">
              ...
              {loading && rows.length === 0 && (
                <div className="text-2xs text-muted-foreground text-center py-6">Loading…</div>
              )}
            </div>
          </div>
        );
 ...
 */
export function KanbanCardAnatomy({
  title,
  metaLines = [],
  chip,
  warning,
  ...
  return (
    <>
      ...
      {metaLines.map((line, i) => (
        <div key={i} className="text-3xs text-muted-foreground mt-0.5">
          {line}
        </div>
      ))}
