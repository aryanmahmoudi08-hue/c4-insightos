...
export function KpiTile({
  ...
  const isZero =
    empty !== undefined ? empty : numericValue !== undefined ? numericValue === 0 : value === 0;
  ...
  const glowStyle = spectrum
    ? ({ "--glow": spectrumGlowStyle(spectrum).boxShadow } as CSSProperties)
    : undefined;

  return (
    <div
      className={cn(
        "hover-lift group relative rounded-md border border-border bg-card overflow-hidden flex flex-col",
        spectrum && "hover:shadow-[var(--glow)]",
      )}
      style={glowStyle}
    >
      <div
      ...
      <div
        className={cn(
          "flex-1 flex flex-col justify-center px-3 gap-1 min-h-[64px]",
          weight === "featured" ? "py-4" : "py-3",
        )}
      >
        {isZero && emptyHint ? (
          <div className="flex flex-col items-center justify-center text-center gap-1 py-1">
            <div className="text-lg font-sans tabular-nums text-muted-foreground/40">—</div>
            ...
            <div className="flex items-center justify-between gap-2">
              <div
                className={cn(
                  ...
                  weight === "featured" ? "text-2xl" : "text-lg",
                  spectrum && SPECTRUM_TEXT_CLASS[spectrum],
                )}
              >
                {display}
              </div>
              {hasDelta && (
                <span
                  className={cn(
                    "badge-glass shrink-0 font-sans tabular-nums text-3xs normal-case tracking-normal",
