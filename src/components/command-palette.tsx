...
import {
  LayoutDashboard, Users, MessageSquare, PhoneCall, PhoneIncoming, CalendarDays,
  UserPlus, GitBranch, TrendingUp, Video, Radar, BadgeCheck, Brain, Sparkles,
  Activity, Settings, ShieldCheck, Plug, Wand2, Layers, Search, Trophy, Award, DollarSign,
} from "lucide-react";
...
const ROUTES: { to: string; label: string; group: string; icon: typeof LayoutDashboard }[] = [
  ...
  { to: "/clients", label: "Clients", group: "Go to", icon: BadgeCheck },
  { to: "/onboarding", label: "Onboarding", group: "Go to", icon: Brain },
  { to: "/fulfillment", label: "Client Results", group: "Go to", icon: BadgeCheck },
  { to: "/vsl", label: "VSL Analytics", group: "Go to", icon: Video },
  { to: "/insights", label: "AI Insights", group: "Go to", icon: Sparkles },
  ...
  { to: "/connectors", label: "Connectors", group: "Go to", icon: Plug },
];

const ACTIONS: { to: string; search: Record<string, string>; label: string; icon: typeof LayoutDashboard }[] = [
  { to: "/dm-setter", search: { action: "log-day" }, label: "Log Day (DM Setter)", icon: Trophy },
  { to: "/inbound-dialer", search: { action: "log-day" }, label: "Log Day (Inbound Dialer)", icon: Trophy },
  { to: "/closer", search: { action: "log-call" }, label: "Log Call", icon: Award },
  { to: "/fulfillment", search: { action: "log-win" }, label: "Log Win (Daily W)", icon: DollarSign },
];

type SearchResult = { id: string; label: string; sub: string; to: string; search: Record<string, string> };
...
export function CommandPalette() {
  ...
  const runSearch = useCallback(async (term: string) => {
    if (!orgId || term.trim().length < 2) { setResults([]); return; }
    setSearching(true);
    const q = term.trim();
    try {
      const [leads, clients, content] = await Promise.all([
        supabase.from("leads").select("id, full_name, handle, email").eq("org_id", orgId)
          .or(`full_name.ilike.%${q}%,handle.ilike.%${q}%,email.ilike.%${q}%`).limit(5),
        supabase.from("clients").select("id, full_name, email").eq("org_id", orgId)
          .or(`full_name.ilike.%${q}%,email.ilike.%${q}%`).limit(5),
        supabase.from("content_pieces").select("id, title").eq("org_id", orgId)
          .ilike("title", `%${q}%`).limit(5),
      ]);
      const out: SearchResult[] = [];
      for (const l of leads.data ?? []) out.push({ id: `lead-${l.id}`, label: l.full_name || l.handle || l.email || "Lead", sub: "Lead", to: "/leads", search: { q: l.full_name || l.handle || "" } });
      for (const c of clients.data ?? []) out.push({ id: `client-${c.id}`, label: c.full_name, sub: "Client", to: "/clients", search: { q: c.full_name } });
      for (const p of content.data ?? []) out.push({ id: `content-${p.id}`, label: p.title || "(untitled)", sub: "Content", to: "/content", search: {} });
      setResults(out);
    } finally {
      setSearching(false);
    }
          ...
          >
            ...
            >
              <div className="flex items-center gap-2 border-b border-border/70 px-3.5">
                ...
                />
                <span className="badge-glass shrink-0 text-3xs normal-case tracking-normal">ESC</span>
              </div>
              <Command.List className="max-h-[50vh] overflow-y-auto p-2">
                ...
                {results.length > 0 && (
                  <Command.Group heading="Results" className="px-2 pb-1 pt-2 text-3xs font-semibold uppercase tracking-wider text-muted-foreground [&_[cmdk-group-heading]]:px-1 [&_[cmdk-group-heading]]:pb-1.5">
                    {results.map((r) => (
                      <Command.Item
                        key={r.id}
                      ...
                      >
                        ...
                        <span className="text-3xs uppercase tracking-wide text-muted-foreground">{r.sub}</span>
                      </Command.Item>
                    ))}
                  </Command.Group>
                )}

                <Command.Group heading="Quick actions" className="px-2 pb-1 pt-2 text-3xs font-semibold uppercase tracking-wider text-muted-foreground [&_[cmdk-group-heading]]:px-1 [&_[cmdk-group-heading]]:pb-1.5">
                  {ACTIONS.map((a) => (
                    <Command.Item
                      key={a.label}
                      value={a.label}
                ...
                <Command.Group heading="Go to" className="px-2 pb-1 pt-2 text-3xs font-semibold uppercase tracking-wider text-muted-foreground [&_[cmdk-group-heading]]:px-1 [&_[cmdk-group-heading]]:pb-1.5">
                  {ROUTES.map((r) => (
                    <Command.Item
                      key={r.to}
                      value={r.label}
