...
export function DailyWinsPanel({
  ...
  const energyRows = rows.filter((w) => w.energy_score != null);
  const avgEnergy = energyRows.length
    ? energyRows.reduce((s, w) => s + (w.energy_score ?? 0), 0) / energyRows.length
    : 0;
      ...
      >
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              ...
              <div className="mt-0.5 text-sm font-semibold">
                {isExternalForm
                  ? formRole
                    ? `${formRole} Typeform assigned`
                    : "Typeform assigned"
                  : "InsightOS fallback form active"}
              </div>
              <div className="mt-1 text-3xs text-muted-foreground">
                {isExternalForm
                  ? "Resolved user → role → default. Credentials remain outside the client."
                  : "Configure role or user Typeforms in Settings, or keep using the existing Daily W form."}
              </div>
            </div>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <a
              href={assignedFormUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-md border border-border px-2.5 py-1.5 text-2xs font-medium hover:bg-muted/40"
            >
              Open form
            </a>
            {isExternalForm && (
              <Button
                ...
                onClick={() => setEmbedForm((value) => !value)}
              >
                {embedForm ? "Hide embed" : "Embed here"}
              </Button>
            )}
          </div>
        </div>
        {embedForm && isExternalForm && (
          <iframe
            ...
            className="mt-4 h-[720px] w-full rounded-lg border border-border bg-background"
            loading="lazy"
          />
        )}
      </div>
