...
const FMT_TONE: Record<string, string> = {
  short_form:
    "bg-[oklch(0.7_0.09_40/15%)] text-[oklch(0.76_0.09_40)] border-[oklch(0.7_0.09_40/30%)]",
  long_form:
    "bg-[oklch(0.7_0.09_76/15%)] text-[oklch(0.76_0.09_76)] border-[oklch(0.7_0.09_76/30%)]",
  long_to_short:
    "bg-[oklch(0.7_0.09_112/15%)] text-[oklch(0.76_0.09_112)] border-[oklch(0.7_0.09_112/30%)]",
  story:
    "bg-[oklch(0.7_0.09_148/15%)] text-[oklch(0.76_0.09_148)] border-[oklch(0.7_0.09_148/30%)]",
  carousel:
    "bg-[oklch(0.7_0.09_184/15%)] text-[oklch(0.76_0.09_184)] border-[oklch(0.7_0.09_184/30%)]",
  email:
    "bg-[oklch(0.7_0.09_215/15%)] text-[oklch(0.76_0.09_215)] border-[oklch(0.7_0.09_215/30%)]",
};
          ...
          return (
            <div key={s.label} className="space-y-1">
              <div className="flex items-center justify-between text-2xs">
                ...
                <span
                  className={`font-sans tabular-nums font-semibold ${SPECTRUM_TEXT_CLASS[s.spectrum]}`}
                >
                  {s.value}
                </span>
              </div>
...
function ContentCalendar() {
  ...
  const { data: pieces = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("content_pieces")
        .select(
          "id, title, platform, hook, body, cta, funnel_stage, angle, topic, pipeline_status, scheduled_date, scheduled_time, post_format, repurpose_plan, voice_notes, why_it_works, posting_instructions",
        )
        .eq("org_id", orgId!)
  ...
  const markPosted = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase
        .from("content_pieces")
        .update({ pipeline_status: "posted", posted_at: new Date().toISOString() })
        .eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const weeks = useMemo(() => {
    ...
    for (const c of cells)
      c.items.sort((a, b) => (a.scheduled_time ?? "").localeCompare(b.scheduled_time ?? ""));
  ...
  const monthPieces = pieces.filter(
    (p) => (p.scheduled_date ?? "").slice(0, 7) === iso(cursor).slice(0, 7),
  );
  ...
  return (
    <>
      <TopBar
        ...
        subtitle="Every approved post, scripted and scheduled — nothing left to figure out"
      />
          ...
          <BentoCell span="tall">
            <div className="flex h-full flex-col gap-4 rounded-2xl border border-border bg-card p-4">
              <div className="flex items-center gap-2">
                <div className="grid h-8 w-8 shrink-0 place-items-center rounded-md bg-muted/60 text-foreground">
              ...
              <div className="flex flex-1 flex-col justify-center">
                <div className="font-sans text-4xl font-bold tabular-nums">
                  {
                    monthPieces.filter(
                      (p) => p.post_format === "long_form" || p.post_format === "long_to_short",
                    ).length
                  }
                </div>
          ...
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            ...
            <div className="flex gap-2 flex-wrap">
              {Object.entries(FORMAT_LABEL).map(([k, v]) => (
                <span key={k} className={`text-3xs rounded border px-1.5 py-0.5 ${FMT_TONE[k]}`}>
                  {v}
                </span>
              ))}
            </div>
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        {slot.items.map((p) => (
                          ...
                          >
                            <div className="flex items-center gap-1 text-4xs font-sans tabular-nums opacity-80">
                              <Clock className="h-2.5 w-2.5" />
                              {(p.scheduled_time ?? "").slice(0, 5) || "—"}
                              {p.pipeline_status === "posted" && (
                                <Check className="h-2.5 w-2.5 ml-auto" />
                              )}
                            </div>
                            <div className="text-3xs font-medium leading-tight line-clamp-2 mt-0.5">
                              {p.title || "(untitled)"}
                            </div>
                          </button>
                        ))}
          ...
          {upcoming.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {upcoming.map((p) => (
                <Card
                  key={p.id}
                  className="p-3 space-y-1.5 cursor-pointer hover:border-accent/50 transition"
                  onClick={() => setOpenPiece(p)}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-4xs rounded border px-1 py-px ${FMT_TONE[p.post_format ?? ""] ?? "bg-muted text-muted-foreground border-border"}`}
                    >
                  ...
                  {p.hook && (
                    <div className="text-2xs text-muted-foreground line-clamp-2 italic">
                      "{p.hook}"
                    </div>
                  )}
                </Card>
            ...
            <>
              <SheetHeader>
                <SheetTitle className="pr-8 leading-snug">
                  {openPiece.title || "(untitled)"}
                </SheetTitle>
              </SheetHeader>
              <div className="mt-4 space-y-4 text-sm">
                <div className="flex flex-wrap gap-1.5">
                  <Badge variant="outline" className="font-sans tabular-nums text-3xs">
                    {openPiece.scheduled_date
                      ? new Date(`${openPiece.scheduled_date}T00:00:00`).toLocaleDateString(
                          undefined,
                          { weekday: "long", month: "long", day: "numeric" },
                        )
                      : "Unscheduled"}
                  </Badge>
                  {openPiece.scheduled_time && (
                    <Badge variant="outline" className="font-sans tabular-nums text-3xs">
                      {openPiece.scheduled_time.slice(0, 5)}
                    </Badge>
                  )}
                  <Badge className="text-3xs">
                    {FORMAT_LABEL[openPiece.post_format ?? ""] ?? openPiece.platform}
                  </Badge>
                  {openPiece.funnel_stage && (
                ...
                <Block icon={FileText} title="Full script">
                  <pre className="whitespace-pre-wrap font-sans text-sm leading-relaxed">
                    {openPiece.body || "No script written yet."}
                  </pre>
                  <Button
                    size="sm"
                    variant="outline"
                    className="mt-2"
                    onClick={() => {
                      navigator.clipboard.writeText(openPiece.body ?? "");
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  ...
                  {openPiece.pipeline_status !== "posted" && (
                    <Button
                      size="sm"
                      onClick={() => markPosted.mutate(openPiece.id)}
