  ...
  head: () => ({
    meta: [
      { title: "Content Signals — What to post next | C4 InsightOS" },
      {
        name: "description",
        content:
          "One brain that merges VSL analytics, FAQ clicks, setting-call objections and client intakes to decide which content mechanism to double down on.",
      },
      { property: "og:title", content: "Content Signals — What to post next" },
      {
        property: "og:description",
        content:
          "Demand mix across the 4 conversion mechanisms, weekly posting check, and root-cause bottleneck reads.",
      },
...
function ContentSignalsPage() {
  ...
  const toggleMix = (k: MechanismKey) =>
    setExpandedMix((prev) => {
  ...
  const {
    ...
    isLoading,
    isError: demandError,
    error: demandErrorObj,
  } = useQuery({
    queryKey: ["content-demand", range.from, range.to, devBypass],
    queryFn: () =>
      devBypass
        ? Promise.resolve(mockContentDemand())
  ...
  const weekly = weeklyData ?? {
    ...
    >,
    reels: 0,
    missing: [] as MechanismKey[],
    untracked: 0,
    best: null as MechanismKey | null,
    worst: null as MechanismKey | null,
  ...
  return (
    <>
      ...
      <main className="p-4 md:p-6 space-y-5 max-w-[1400px]">
        <header>
          <h1 className="display-serif text-2xl flex items-center gap-2">
            <Radar className="h-5 w-5" /> What to post next
          </h1>
          <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
            One brain. FAQ video clicks, setting-call objections, client intake answers, VSL
            drop-off and reel performance all collapse into a single posting mix. Cash inconsistency
            → unknown posting → untracked performance. Fix tracking, fix everything.
          </p>
        </header>
        ...
        <BentoGrid cols={2} rowHeight="9.5rem">
          <BentoCell span="wide">
            {weeklyError ? (
              ...
              <RootCauseChain
                untaggedCount={weekly.per["untagged"]?.count ?? 0}
                untrackedCount={weekly.untracked}
                totalPosts={weekly.total}
              />
            )}
          </BentoCell>
        </BentoGrid>
        ...
        <Card className="p-4 space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <div className="text-xs uppercase tracking-wider text-muted-foreground">
                Recommended mix · {range.from} → {range.to}
              </div>
              {demand?.insufficientData && (
                <span
                  ...
                  title={`Total signal weight ${demand.totalWeight} is below the configured minimum of ${demand.minTotalWeight} — this mix is a real computed split, not a placeholder, but it's resting on thin signal. Log more FAQ clicks, setter calls, or intakes to sharpen it.`}
                >
            ...
            <div className="flex items-center gap-2 text-2xs text-muted-foreground">
              ...
              <Input
                ...
                max={21}
                value={reelTarget}
                onChange={(e) => setReelTarget(Math.max(1, Number(e.target.value) || 1))}
          ...
          {isLoading ? (
            ...
            <EmptyState
              ...
              title="No mix yet"
              description="Waiting on a workspace to compute a mix against."
            />
          ) : (
              ...
              {MECHANISM_KEYS.map((k) => {
                ...
                return (
                  <div
                    key={k}
                    id={`mechanism-${k}`}
                    className="rounded-md border border-border p-3 space-y-1.5 scroll-mt-20"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">{MECHANISMS[k].label}</span>
                      <span className="font-sans tabular-nums text-sm">{pct}%</span>
                    </div>
          ...
          {demand && (
            <div className="text-2xs text-muted-foreground">
              Built from {demand.counts.faq} FAQ videos · {demand.counts.setter_calls} setting-call
              signals · {demand.counts.intakes} client intakes · {demand.counts.reels} content
              pieces.
            </div>
          )}
        </Card>
        ...
        <Card className="p-4 space-y-3">
          ...
          {weeklyError ? (
            <QueryErrorCard label="Weekly check" error={weeklyErrorObj} />
          ) : (
            <>
              <div className="grid gap-2 sm:grid-cols-3">
                ...
                <Stat
                  ...
                  value={
                    weekly.missing.length === 0 ? "Yes" : `No — ${weekly.missing.length} missing`
                  }
                  sub={
                    weekly.missing.length
                      ? weekly.missing.map((k) => MECHANISMS[k].label).join(", ")
                      : "full coverage"
                  }
              ...
              <div className="grid gap-2 md:grid-cols-2">
                <div className="rounded-md border border-[color:var(--color-success)]/45 bg-[color:var(--color-success)]/5 p-3 text-xs space-y-1">
                  ...
                  {weekly.best && weekly.per[weekly.best].count > 0 ? (
                    <div>
                      {MECHANISMS[weekly.best].label} — {weekly.per[weekly.best].dms} DMs,{" "}
                      {weekly.per[weekly.best].calls} calls booked, $
                      {Math.round(weekly.per[weekly.best].cash / 100).toLocaleString()} cash. Double
                      down.
                    </div>
                  ) : (
                    <div className="text-muted-foreground">
                      No performance logged yet this week.
                    </div>
                  )}
                </div>
                ...
                >
                  <div
                    className={cn(
                      "flex items-center gap-1.5 font-semibold uppercase tracking-wider text-3xs",
                      weekly.worstDiagnosis?.label === "Underperforming"
                        ? "text-destructive"
        ...
        <Card className="p-4 space-y-2">
          ...
          {insight ? (
            <div className="text-xs leading-relaxed whitespace-pre-wrap animate-in fade-in-0 slide-in-from-top-1 duration-300">
              {insight}
            </div>
          ) : (
            <p className="text-xs text-muted-foreground">
              Merges VSL play/drop-off, FAQ clicks, setting-call objections, intake answers and reel
              performance into a root-cause chain plus this week's 5-7 reels.
            </p>
          )}
        </Card>
...
function RootCauseChain({
  ...
  const nodes: { key: string; label: string; tone: ChipTone }[] = [
    { key: "cash", label: "Cash inconsistent", tone: "destructive" },
    {
      ...
      label:
        untaggedCount > 0
          ? `${untaggedCount} ${post(untaggedCount)} untagged`
          : "Posting mix tracked",
      tone: untaggedCount > 0 ? "destructive" : "success",
    },
    {
      key: "perf",
      label:
        untrackedCount > 0
          ? `${untrackedCount} ${post(untrackedCount)} w/o metrics`
          : "Performance tracked",
      tone: untrackedCount > 0 ? "destructive" : "success",
    },
      ...
      <div className="relative flex flex-wrap items-center gap-1.5">
        {nodes.map((n, i) => (
          <div key={n.key} className="flex items-center gap-1.5">
            <div
              className={cn(
                "flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-2xs font-medium",
                n.tone === "destructive" &&
      ...
      <div className="relative text-2xs text-muted-foreground">
        {gapNode ? (
          <>
            Current gap:{" "}
            <span
              className={cn("font-medium", gapNode.tone === "destructive" && "text-destructive")}
            >
              {gapNode.label}
            </span>{" "}
            out of {totalPosts} posts this week — fix that first, every downstream read (mix %,
            bottleneck engine, double-down calls) sharpens automatically once it's closed.
          </>
        ) : (
          <>
  ...
  return (
    <Card className="p-4 space-y-3">
      ...
      <p className="text-2xs text-muted-foreground">
        Paste the setting-call transcript (or the setter's notes from Close). AI pulls the limiting
        beliefs and objections, then decides which mechanism kills them.
      </p>
      <div className="grid gap-2 sm:grid-cols-3">
...
function QueryErrorCard({ label, error }: { label: string; error: unknown }) {
  ...
  return (
    <div className="rounded-md border border-destructive/45 bg-destructive/5 p-3 text-xs space-y-1">
      ...
      <div className="text-muted-foreground">
        This is a fetch failure, not a "not enough data" read — nothing below should be trusted
        until this is fixed. {message}
      </div>
    </div>
  );
}
...
function Stat({
  ...
  return (
    <div className="rounded-md border border-border p-3">
      ...
      <div
        className={cn(
          "mt-0.5 font-sans tabular-nums text-lg font-semibold",
