...
function twiml(status = 200) {
  return new Response("<?xml version=\"1.0\" encoding=\"UTF-8\"?><Response></Response>", { status, headers: TWIML_HEADERS });
}
...
/**
 * Twilio signs form-encoded callbacks with Base64(HMAC-SHA1(authToken,
 * callbackUrl + sorted key/value parameters)). This implementation uses the
 * Cloudflare/Web Crypto API, which keeps validation server-side without
 * bundling the full Node client SDK into the worker.
 */
async function validateTwilioSignature(authToken: string, signature: string, url: string, payload: Record<string, string>) {
  const canonical = url + Object.keys(payload).sort().map((key) => `${key}${payload[key]}`).join("");
...
function externalEventId(event: string, payload: Record<string, string>) {
  ...
  const state = payload.RecordingStatus ?? payload.MessageStatus ?? payload.CallStatus ?? "received";
...
async function recordExternalEvent(input: { orgId: string; event: string; eventId: string; payload: Record<string, string> }) {
  const db = supabaseAdmin as any;
  ...
  const { data, error } = await db
    .from("crm_external_events")
    .insert({ org_id: input.orgId, provider: "twilio", external_event_id: input.eventId, event_type: input.event, processing_status: "received", payload: input.payload })
    .select("id")
    .single();
  if (error) {
...
async function resolveContactContext(orgId: string, phone: string | undefined) {
  ...
  const [contact, lead] = await Promise.all([
    db.from("crm_contacts").select("id").eq("org_id", orgId).eq("primary_phone", phone).maybeSingle(),
...
async function appendActivity(input: { orgId: string; type: string; title: string; sourceId: string; contactId?: string | null; legacyLeadId?: string | null; payload: Record<string, string> }) {
  const db = supabaseAdmin as any;
  const { data: activity, error } = await db
    .from("crm_activities")
    .insert({ org_id: input.orgId, activity_type: input.type, source_type: "twilio", source_id: input.sourceId, title: input.title, payload: input.payload })
    .select("id")
    .single();
  if (error) throw new Error(error.message);
  const targets = [
    input.contactId ? { org_id: input.orgId, activity_id: activity.id, entity_type: "contact", entity_id: input.contactId } : null,
    input.legacyLeadId ? { org_id: input.orgId, activity_id: activity.id, entity_type: "legacy_lead", entity_id: input.legacyLeadId } : null,
  ].filter(Boolean);
  if (targets.length) {
...
async function handleSms(account: { id: string; org_id: string }, payload: Record<string, string>) {
  ...
  if (!threadId) {
    ...
    const { error: updateError } = await db.from("crm_communication_threads").update({ unread_count: 1, last_message_at: new Date().toISOString() }).eq("id", threadId);
...
async function handleVoice(account: { id: string; org_id: string }, payload: Record<string, string>) {
  const db = supabaseAdmin as any;
  const context = await resolveContactContext(account.org_id, payload.From);
  const callStatus = (payload.CallStatus ?? "initiated").replace(/-/g, "_");
  const normalizedStatus = ["queued", "initiated", "ringing", "answered", "completed", "busy", "failed", "no_answer", "canceled"].includes(callStatus) ? callStatus : "initiated";
  const { data: call, error } = await db
    ...
    .upsert({
      ...
      direction: payload.Direction === "outbound-api" || payload.Direction === "outbound-dial" ? "outbound" : "inbound",
      status: normalizedStatus,
      from_address: payload.From ?? null,
      to_address: payload.To ?? null,
      started_at: payload.Timestamp ? new Date(payload.Timestamp).toISOString() : new Date().toISOString(),
...
async function handleRecording(account: { id: string; org_id: string }, payload: Record<string, string>) {
  const db = supabaseAdmin as any;
  ...
  const status = payload.RecordingStatus === "completed" ? "available" : payload.RecordingStatus === "absent" ? "absent" : payload.RecordingStatus === "failed" ? "failed" : "processing";
  ...
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        ...
        if (!event || !SUPPORTED_EVENTS.has(event)) return json({ error: "Unsupported Twilio event" }, 404);
        const authToken = process.env.TWILIO_AUTH_TOKEN;
        const configuredUrl = callbackUrl(request);
        if (!authToken || !configuredUrl) return json({ error: "Twilio webhook is not configured" }, 503);

        const form = await request.formData();
        const payload = payloadFrom(form);
        const signature = request.headers.get("x-twilio-signature") ?? "";
        if (!signature || !(await validateTwilioSignature(authToken, signature, configuredUrl, payload))) return json({ error: "Invalid Twilio signature" }, 401);

        const eventId = externalEventId(event, payload);
        if (!eventId) return json({ error: "Twilio callback has no external event identifier" }, 400);
        try {
          ...
          const externalEvent = await recordExternalEvent({ orgId: account.org_id, event, eventId, payload });
          ...
          await (supabaseAdmin as any).from("crm_external_events").update({ processing_status: "processed", processed_at: new Date().toISOString() }).eq("id", externalEvent.id);
          return twiml();
        } catch (error) {
