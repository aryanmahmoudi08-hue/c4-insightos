          ...
          <div className="divide-y divide-border max-h-[70vh] overflow-y-auto">
            {(clients ?? [])
              .filter((c) => {
                ...
                if (!q) return true;
                return [c.full_name, c.email, c.offer_name].some((v) =>
              ...
              .map((c) => (
                <button
                  ...
                  className={`w-full text-left p-3 hover:bg-muted/30 ${selected === c.id ? "bg-muted/40" : ""}`}
                >
      ...
      {!activeClient && (
        <div className="px-6 pb-6 space-y-4">
          <div className="rounded-lg border border-border bg-card p-4">
            <DailyWinsPanel formRole="client" />
          </div>
        </div>
      )}
    </>
  ...
  return (
    <div className="rounded-md border border-border p-3">
      ...
      <div className="text-sm whitespace-pre-wrap min-h-[2.5rem]">
        {value || <span className="text-muted-foreground italic">No intake answer</span>}
      </div>
    </div>
  );
}
