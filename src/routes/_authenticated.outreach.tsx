...
function MessagePipelineHero({
  counts,
  lists,
}: {
  counts: { draft: number; scheduled: number; sending: number; sent: number; failed: number };
  lists: number;
}) {
...
function Outreach() {
  ...
  const { data: messages } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, kind, subject, body, scheduled_for, sent_at, status, list_id, created_at, outreach_lists(name)",
        )
        .eq("org_id", orgId!)
  ...
  const createList = useMutation({
    mutationFn: async (form: FormData) => {
      ...
      const { error } = await supabase
        .from("outreach_lists")
        .insert({ org_id: orgId!, name, kind });
      if (error) throw error;
    },
    onSuccess: () => {
      toast.success("List created");
      qc.invalidateQueries({ queryKey: ["outreach-lists"] });
      setListOpen(false);
    },
  ...
  const createMessage = useMutation({
    ...
    onSuccess: () => {
      toast.success("Message saved · queued for next cron tick");
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex justify-between items-center">
          <div className="text-xs text-muted-foreground">
            {lists?.length ?? 0} lists · {messages?.length ?? 0} messages
          </div>
          <div className="flex gap-2">
            <Dialog open={listOpen} onOpenChange={setListOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Create outreach list</DialogTitle>
                </DialogHeader>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                ...
                >
                  <div className="space-y-1.5">
                    <Label>Name</Label>
                    <Input name="name" placeholder="e.g. June warm leads" required />
                  </div>
                  <div className="space-y-1.5">
                    <Label>Channel</Label>
            ...
            <Dialog open={composeOpen} onOpenChange={setComposeOpen}>
              ...
              <DialogContent className="max-w-xl">
                ...
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    createMessage.mutate(new FormData(e.currentTarget));
                  }}
                  className="space-y-3"
                >
                  ...
                  <div className="space-y-1.5">
                    <Label>Body</Label>
                    <Textarea name="body" rows={8} placeholder="Write your message…" required />
                  </div>
                  <div className="space-y-1.5">
                    <Label>Schedule for (leave blank to save as draft)</Label>
                  ...
                  <p className="text-2xs text-muted-foreground">
                    Messages are queued in the database. A scheduled cron tick processes{" "}
                    <span className="font-mono">queued</span> + due{" "}
                    <span className="font-mono">scheduled</span> rows through your email / SMS
        ...
        <Tabs defaultValue="messages">
            ...
            <div className="rounded-lg border border-border bg-card overflow-hidden">
              ...
              {(messages ?? []).length === 0 && (
                <div className="p-10 text-center text-xs text-muted-foreground">
                  ...
                  No messages yet — click <span className="font-semibold">Compose</span> to draft
                  your first one.
                </div>
              )}
              {(messages ?? []).map((m) => (
                <div
                  key={m.id}
                  className="border-t border-border px-3 py-2 grid grid-cols-12 gap-2 items-center text-sm hover:bg-muted/20"
                >
                  ...
                  <div className="col-span-1">
                    <span
                      className={`inline-block rounded px-1.5 py-0.5 text-3xs font-mono uppercase ${STATUS_TONE[m.status] ?? "bg-muted text-muted-foreground"}`}
                    >
                      {m.status}
                    </span>
                  </div>
          ...
          <TabsContent value="lists">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {(lists ?? []).map((l) => {
                const count =
                  (l.outreach_recipients as { count: number }[] | null)?.[0]?.count ?? 0;
                return (
