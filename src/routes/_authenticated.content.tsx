...
function ContentIntel() {
  ...
  const { data: pieces } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("content_pieces")
        .select(
          "id, title, platform, hook, angle, posted_at, url, funnel_stage, body, pipeline_status, mechanism, variation, variation_answers, content_metrics(captured_at, views, reach, likes, leads_generated, closes, cash_collected_cents, hook_retention_pct, comments, shares, saves, followers_gained, profile_visits, dms_generated, calls_booked, watch_time_seconds, avg_watch_pct, follower_views, non_follower_views, engagement_rate_pct, drop_off_rate_pct)",
        )
        .eq("org_id", orgId!)
  ...
  const perf = useMemo(() => {
    ...
    const now = Date.now();
    const postedThisWeek = list.filter(
      (p) => p.posted_at && now - new Date(p.posted_at).getTime() < 7 * 86400e3,
    ).length;
  ...
  const { data: demand } = useQuery({
    queryKey: ["content-command-demand", orgId, devBypass],
    enabled: devBypass || !!orgId,
    queryFn: () =>
      devBypass ? Promise.resolve(mockContentDemand()) : demandFn({ data: { days: 30 } }),
    retry: false,
  });
  ...
  return (
    <>
      <TopBar
        ...
        subtitle="Performance, audience signals, retention, and attributed cash"
      />
      <div className="mx-auto max-w-[1680px] space-y-4 p-4 md:p-7">
        <PageHero
          ...
          eyebrow="ContentOS"
          title="Content Command Center"
          subtitle="Every post, signal, and conversion path in one operating view — with the existing Content Intelligence workspace preserved below."
          status={[
            { label: `${pieces?.length ?? 0} pieces tracked`, tone: "default" },
            { label: `${perf.postedThisWeek} posted this week`, tone: "success" },
            ...(perf.inReview > 0
              ? [{ label: `${perf.inReview} in review`, tone: "warning" as const }]
              : []),
          ]}
          stats={[
        ...
        <ContentCommandCenter
          pieces={(pieces ?? []) as unknown as ContentCommandPiece[]}
          demand={demand as ContentDemandSummary | undefined}
          weekly={weekly as ContentWeeklySummary | undefined}
        />

        {/* Existing Content Intelligence performance snapshot */}
        <div className="grid grid-cols-2 gap-3 stagger-fade md:grid-cols-4">
          <MetricCard
            label="Views"
            value={fmtN(perf.totals.views)}
            icon={<Eye className="h-3 w-3" />}
            spectrum="cold"
            deltaPct={12}
            spark={[40, 55, 48, 62, 70, 65, 80]}
          />
          <MetricCard
            label="Leads generated"
            value={fmtN(perf.totals.leads)}
          ...
          />
          <MetricCard
            label="Closes attributed"
            value={fmtN(perf.totals.closes)}
          ...
          <MetricCard
            ...
            value={"$" + fmtN(perf.totals.cash / 100)}
            ...
            <div className="hover-lift relative flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card p-5">
              ...
              <div className="relative flex-1 flex flex-col justify-center space-y-2 py-3">
                {(
                  ...
                  ] as { stage: string; value: number; spectrum: SpectrumPosition }[]
                ).map((f, i, arr) => {
                    ...
                    <div key={f.stage} className="space-y-0.5">
                      ...
                      <div className="h-5 rounded bg-muted/30 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${width}%`, background: SPECTRUM_VAR[f.spectrum] }}
                        />
                      </div>
                    </div>
                  );
          ...
          <BentoCell span="tall">
            <div className="flex h-full flex-col rounded-2xl border border-border bg-card p-4 shadow-sm">
              <div className="flex items-center justify-between gap-2 border-b border-border/60 pb-3">
                ...
                <span className="badge-glass text-3xs normal-case tracking-normal">
                  {perf.topHooks.length} patterns
                </span>
              </div>
              <div className="mt-3 space-y-2.5">
                {perf.topHooks.map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className="hover-lift rounded-xl border border-border bg-background/60 p-3"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="text-xs font-medium leading-snug line-clamp-2">
                          {p.title || p.hook || "(untitled)"}
                        </div>
                        <span
                          className={`badge-glass shrink-0 normal-case tracking-normal ${retention >= 55 ? "text-[color:var(--color-success)]" : "text-muted-foreground"}`}
                        >
                ...
                {perf.topHooks.length === 0 && (
                  <div className="py-8 text-center text-xs italic text-muted-foreground">
                    Log content with hook retention % to see the strongest patterns here.
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-4 shadow-sm md:p-5">
          ...
          <HeatmapGrid
            rowLabels={perf.heatmapRows}
            colLabels={perf.heatmapCols}
            data={perf.heatmapData}
            valueFmt={(v) => fmtN(v) + " views"}
            variant="spectrum"
          />
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="space-y-1.5">
          <Label>Views</Label>
          <Input name="views" type="number" defaultValue={prefill?.views ?? 0} />
        </div>
        <div className="space-y-1.5">
          <Label>Reach</Label>
          <Input name="reach" type="number" defaultValue={prefill?.reach ?? 0} />
        </div>
        <div className="space-y-1.5">
          <Label>Leads</Label>
          <Input name="leads" type="number" defaultValue={prefill?.leads ?? 0} />
        </div>
        <div className="space-y-1.5">
          <Label>Hook retention %</Label>
          <Input name="retention" type="number" step="0.1" defaultValue={prefill?.retention ?? 0} />
        </div>
      </div>
      <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-4 gap-3">
          <div className="space-y-1.5">
            ...
            <Input name="likes" type="number" defaultValue={prefill?.likes ?? 0} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Comments</Label>
            <Input name="comments" type="number" defaultValue={prefill?.comments ?? 0} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Shares</Label>
            <Input name="shares" type="number" defaultValue={prefill?.shares ?? 0} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Saves</Label>
            <Input name="saves" type="number" defaultValue={prefill?.saves ?? 0} />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Input name="follower_views" type="number" defaultValue={prefill?.followerViews ?? 0} />
          </div>
          <div className="space-y-1.5">
