...
export function SalesCrm() {
  ...
  const updatePipelineStageFn = useServerFn(updateCrmPipelineStage);
  ...
  const pipelineStageUpdateMutation = useMutation({
    mutationFn: (data: Parameters<typeof updatePipelineStageFn>[0]) => updatePipelineStageFn(data),
    onSuccess: async () => { toast.success("Pipeline stage updated"); await invalidate(); },
    onError: (error: Error) => toast.error(error.message),
  });
      ...
      <main className="mx-auto max-w-[1760px] space-y-5 p-4 sm:p-6">
        ...
        <Tabs defaultValue="contacts" className="space-y-4">
          ...
          <TabsContent value="pipeline" className="mt-0">
            <PipelineBoard pipelines={overview.pipelines as Pipeline[]} opportunities={overview.opportunities as Opportunity[]} canManage={canManage} onMove={(id, pipeline_stage_id) => opportunityStageMutation.mutate({ data: { id, pipeline_stage_id } })} onUpdateStage={(data) => pipelineStageUpdateMutation.mutate({ data })} stagePendingId={pipelineStageUpdateMutation.isPending ? pipelineStageUpdateMutation.variables?.data.id : undefined} />
          </TabsContent>
...
function PipelineBoard({ pipelines, opportunities, canManage, onMove, onUpdateStage, stagePendingId }: { pipelines: Pipeline[]; opportunities: Opportunity[]; canManage: boolean; onMove: (id: string, stageId: string) => void; onUpdateStage: (data: { id: string; name: string; probability: number; color?: string | null; is_closed_won: boolean; is_closed_lost: boolean }) => void; stagePendingId?: string }) {
  const stages = pipelines.flatMap((pipeline) => ((pipeline.crm_pipeline_stages ?? []) as Array<{ id: string; name: string; position: number; probability: number; color?: string | null; is_closed_won: boolean; is_closed_lost: boolean }>).sort((a, b) => a.position - b.position).map((stage) => ({ ...stage, pipeline_id: pipeline.id, pipeline_name: pipeline.name })));
  ...
  return (
    <section className="space-y-4">
      {pipelines.map((pipeline) => {
        ...
        return <div key={pipeline.id} className="overflow-hidden rounded-xl border border-border/80 bg-card"><div className="flex items-center justify-between border-b border-border/70 px-4 py-3"><div><h2 className="font-semibold">{pipeline.name}</h2><p className="text-2xs text-muted-foreground">{pipeline.description ?? "Configurable sales pipeline"}</p></div>{pipeline.is_default && <span className="rounded bg-accent/15 px-2 py-1 text-3xs font-semibold uppercase tracking-wide text-accent">Default</span>}</div><div className="grid min-w-max grid-flow-col auto-cols-[minmax(230px,1fr)] gap-px overflow-x-auto bg-border/70 p-px">{pipelineStages.map((stage) => <PipelineColumn key={stage.id} stage={stage} pipelineStages={pipelineStages} opportunities={opportunities.filter((opportunity) => opportunity.pipeline_stage_id === stage.id)} canManage={canManage} onMove={onMove} onUpdateStage={onUpdateStage} stagePending={stagePendingId === stage.id} />)}</div></div>;
      })}
    </section>
  );
}

function PipelineColumn({ stage, pipelineStages, opportunities, canManage, onMove, onUpdateStage, stagePending }: { stage: { id: string; name: string; probability: number; color?: string | null; is_closed_won: boolean; is_closed_lost: boolean }; pipelineStages: Array<{ id: string; name: string }>; opportunities: Opportunity[]; canManage: boolean; onMove: (id: string, stageId: string) => void; onUpdateStage: (data: { id: string; name: string; probability: number; color?: string | null; is_closed_won: boolean; is_closed_lost: boolean }) => void; stagePending: boolean }) {
  const total = opportunities.reduce((sum, opportunity) => sum + (opportunity.amount_cents ?? 0), 0);
  return <div className="min-h-[300px] bg-card p-3"><div className="mb-3 flex items-start justify-between gap-3"><div><div className="flex items-center gap-1 text-xs font-semibold">{stage.name}{canManage ? <StageEditorDialog stage={stage} used={opportunities.length > 0} pending={stagePending} onSubmit={onUpdateStage} /> : null}</div><div className="mt-0.5 text-2xs text-muted-foreground">{stage.probability}% probability · {opportunities.length} deals</div></div><div className="text-xs font-semibold tabular-nums">{formatMoney(total)}</div></div><div className="space-y-2">{opportunities.map((opportunity) => <div key={opportunity.id} className="rounded-lg border border-border/80 bg-muted/20 p-3 shadow-sm"><Link to="/sales/opportunities/$id" params={{ id: opportunity.id }} className="text-sm font-medium hover:text-accent hover:underline">{opportunity.name}</Link><div className="mt-1 text-xs tabular-nums text-muted-foreground">{formatMoney(opportunity.amount_cents, opportunity.currency)}</div><div className="mt-3 flex items-center justify-between text-2xs text-muted-foreground"><span>{opportunity.expected_close_date ? `Close ${formatDate(opportunity.expected_close_date)}` : "No close date"}</span><span className="rounded bg-background px-1.5 py-0.5 uppercase">{opportunity.status}</span></div>{canManage && <Select value={opportunity.pipeline_stage_id} onValueChange={(pipeline_stage_id) => onMove(opportunity.id, pipeline_stage_id)}><SelectTrigger className="mt-3 h-7 text-2xs"><SelectValue /></SelectTrigger><SelectContent>{pipelineStages.map((pipelineStage) => <SelectItem key={pipelineStage.id} value={pipelineStage.id}>{pipelineStage.name}</SelectItem>)}</SelectContent></Select>}</div>)}{!opportunities.length && <div className="rounded-lg border border-dashed border-border p-4 text-center text-2xs text-muted-foreground">No opportunities in this stage.</div>}</div></div>;
}

function StageEditorDialog({ stage, used, pending, onSubmit }: { stage: { id: string; name: string; probability: number; color?: string | null; is_closed_won: boolean; is_closed_lost: boolean }; used: boolean; pending: boolean; onSubmit: (data: { id: string; name: string; probability: number; color?: string | null; is_closed_won: boolean; is_closed_lost: boolean }) => void }) {
  const [open, setOpen] = useState(false);
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button type="button" size="icon" variant="ghost" className="h-5 w-5" aria-label={`Edit ${stage.name} stage`}><Pencil className="h-3 w-3" /></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Edit pipeline stage</DialogTitle></DialogHeader><form className="space-y-3" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onSubmit({ id: stage.id, name: String(form.get("name") ?? ""), probability: Number(form.get("probability") ?? 0), color: String(form.get("color") ?? "") || null, is_closed_won: form.get("is_closed_won") === "on", is_closed_lost: form.get("is_closed_lost") === "on" }); setOpen(false); }}><FormField label="Stage name"><Input name="name" defaultValue={stage.name} required maxLength={120} /></FormField><div className="grid grid-cols-[1fr_auto] gap-3"><FormField label="Probability"><Input name="probability" type="number" min="0" max="100" step="1" defaultValue={stage.probability} required /></FormField><FormField label="Color"><Input name="color" type="color" defaultValue={stage.color && /^#[0-9a-fA-F]{6}$/.test(stage.color) ? stage.color : "#2563eb"} className="h-10 w-14 p-1" /></FormField></div><div className="rounded-lg border border-border bg-muted/20 p-3"><label className="flex items-center gap-2 text-sm"><input name="is_closed_won" type="checkbox" defaultChecked={stage.is_closed_won} disabled={used} />Closed won terminal stage</label><label className="mt-2 flex items-center gap-2 text-sm"><input name="is_closed_lost" type="checkbox" defaultChecked={stage.is_closed_lost} disabled={used} />Closed lost terminal stage</label>{used ? <p className="mt-3 text-2xs leading-5 text-muted-foreground">Terminal semantics are locked because this stage already contains opportunities. Create a new stage instead of reclassifying historical deals.</p> : <p className="mt-3 text-2xs leading-5 text-muted-foreground">A stage may be active, closed won, or closed lost, but never both terminal states.</p>}</div><Button type="submit" className="w-full" disabled={pending}>{pending ? "Saving…" : "Save stage"}</Button></form></DialogContent></Dialog>;
}
...
function TaskQueue({ tasks, canManage, pendingId, onStatusChange }: { tasks: Task[]; canManage: boolean; pendingId?: string; onStatusChange: (id: string, status: "open" | "in_progress" | "completed" | "cancelled") => void }) {
  const [scope, setScope] = useState<"active" | "open" | "in_progress" | "completed" | "cancelled">("active");
  const visible = tasks.filter((task) => scope === "active" ? ["open", "in_progress"].includes(task.status) : task.status === scope);
  const now = new Date();
  const dayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
  const dayEnd = dayStart + 86_400_000;
  const groups = scope === "active" ? [
    { title: "Overdue", tasks: visible.filter((task) => task.due_at && new Date(task.due_at).getTime() < dayStart) },
    { title: "Today", tasks: visible.filter((task) => task.due_at && new Date(task.due_at).getTime() >= dayStart && new Date(task.due_at).getTime() < dayEnd) },
    { title: "Upcoming", tasks: visible.filter((task) => task.due_at && new Date(task.due_at).getTime() >= dayEnd) },
    { title: "No due date", tasks: visible.filter((task) => !task.due_at) },
  ] : [{ title: scope.replaceAll("_", " "), tasks: visible }];
  return <section className="overflow-hidden rounded-xl border border-border/80 bg-card"><div className="border-b border-border/70 px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-sm font-semibold">Work queue</h2><p className="text-xs text-muted-foreground">Native CRM tasks, grouped by urgency and explicit lifecycle state.</p></div><div className="flex flex-wrap gap-1">{(["active", "open", "in_progress", "completed", "cancelled"] as const).map((item) => <button key={item} type="button" onClick={() => setScope(item)} className={cn("rounded-md px-2 py-1 text-2xs font-medium capitalize", scope === item ? "bg-accent text-accent-foreground" : "bg-muted/60 text-muted-foreground hover:bg-muted")}>{item.replaceAll("_", " ")}</button>)}</div></div></div><div>{groups.map((group) => group.tasks.length ? <div key={group.title}><div className="border-y border-border/60 bg-muted/20 px-4 py-2 text-2xs font-semibold uppercase tracking-wide text-muted-foreground">{group.title} · {group.tasks.length}</div><div className="divide-y divide-border/60">{group.tasks.map((task) => <TaskQueueRow key={task.id} task={task} canManage={canManage} pending={pendingId === task.id} onStatusChange={onStatusChange} overdue={group.title === "Overdue"} />)}</div></div> : null)}{!visible.length && <div className="p-10 text-center text-sm text-muted-foreground">No {scope === "active" ? "active" : scope.replaceAll("_", " ")} CRM tasks. Create the next follow-up from the quick actions above.</div>}</div></section>;
}
function TaskQueueRow({ task, canManage, pending, onStatusChange, overdue }: { task: Task; canManage: boolean; pending: boolean; onStatusChange: (id: string, status: "open" | "in_progress" | "completed" | "cancelled") => void; overdue: boolean }) {
  return <div className="flex items-center gap-3 px-4 py-3"><span className={cn("h-2 w-2 rounded-full", task.priority === "urgent" ? "bg-destructive" : task.priority === "high" ? "bg-[color:var(--color-warning)]" : "bg-accent")} /><div className="min-w-0 flex-1"><div className="truncate text-sm font-medium">{task.title}</div>{task.description ? <div className="mt-0.5 truncate text-2xs text-muted-foreground">{task.description}</div> : null}<div className="mt-1 text-2xs text-muted-foreground">{task.contact_id ? "Linked contact" : task.opportunity_id ? "Linked opportunity" : task.company_id ? "Linked company" : "General CRM task"}</div></div><div className="text-right text-2xs text-muted-foreground"><div className={cn("flex items-center justify-end gap-1", overdue && "text-destructive")}><Clock3 className="h-3 w-3" />{formatDate(task.due_at)}</div><div className="mt-1 uppercase tracking-wide">{task.priority}</div>{canManage ? <Select value={task.status} disabled={pending} onValueChange={(status) => onStatusChange(task.id, status as "open" | "in_progress" | "completed" | "cancelled")}><SelectTrigger className="mt-2 h-7 w-32 text-2xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="open">Open</SelectItem><SelectItem value="in_progress">In progress</SelectItem><SelectItem value="completed">Completed</SelectItem><SelectItem value="cancelled">Cancelled</SelectItem></SelectContent></Select> : <div className="mt-2 uppercase tracking-wide">{task.status.replaceAll("_", " ")}</div>}</div></div>;
}
