  ...
  head: () => ({
    meta: [
      { title: "Weekly Report | C4 InsightOS" },
      {
        name: "description",
        content:
          "Cash, calls, funnel, content, rep performance, client health and hiring — the past 7 days, one page, real numbers.",
      },
    ],
  }),
});
...
function WeeklyReportPage() {
  ...
  return (
    <>
      <TopBar
        title="Weekly Report"
        subtitle="Cash, calls, funnel, content, reps, clients, hiring — the last 7 days, one page"
      />
      <div className="p-6 space-y-5 max-w-[1400px]">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button
            ...
            className="gap-1.5"
            onClick={() => send.mutate()}
            disabled={send.isPending || !report}
          >
            {send.isPending ? (
        ...
        {report && (
          <div className="space-y-5">
            {report.trends.length > 0 && (
              <div className="rounded-md border border-border bg-card p-4 space-y-1.5">
                <div className="flex items-center gap-1.5 text-2xs font-semibold uppercase tracking-wider text-muted-foreground">
                  <TriangleAlert className="h-3.5 w-3.5" /> What moved this week
                </div>
            ...
            <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
              ...
              <StatCard
                label="Content mix"
                value={report.contentMix.insufficientData ? "Limited data" : "On track"}
                spectrum={report.contentMix.insufficientData ? undefined : "hot"}
                hint={`Weight ${report.contentMix.totalWeight.toFixed(0)} of ${report.contentMix.minTotalWeight} min`}
              />
            </div>
            ...
            <div className="rounded-xl border border-border bg-card p-4 space-y-3">
              <div className="flex items-center gap-1.5 text-2xs font-semibold uppercase tracking-wider text-muted-foreground">
                <Sparkles className="h-3.5 w-3.5" /> Content mix this week
              </div>
              {report.contentMix.insufficientData && (
                <div className="text-xs text-muted-foreground italic">
                  Total signal weight {report.contentMix.totalWeight.toFixed(0)} is below the
                  configured minimum of {report.contentMix.minTotalWeight} — this mix is real, just
                  resting on thin signal.
                </div>
              )}
                ...
                {(Object.keys(report.contentMix.mix) as MechanismKey[]).map((k) => (
                  <div key={k} className="rounded-md border border-border p-2.5">
                    <div className="text-2xs text-muted-foreground">
                      {MECHANISMS[k]?.label ?? k}
                    </div>
                    <div className="font-sans tabular-nums text-lg font-semibold">
                      {report.contentMix.mix[k]}%
                    </div>
                  </div>
                ))}
              </div>
              <div className="text-2xs text-muted-foreground">
                ...
                {report.weeklyContentCheck.missing.length > 0 &&
                  ` · missing: ${report.weeklyContentCheck.missing.map((m) => MECHANISMS[m]?.label ?? m).join(", ")}`}
              </div>
            </div>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-xl border border-border bg-card p-4 space-y-2">
                <div className="text-2xs font-semibold uppercase tracking-wider text-muted-foreground">
                  Client renewal stages
                </div>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  {Object.entries(report.clientHealth.renewalStageBreakdown).map(
                    ([stage, count]) => (
                      <div
                        key={stage}
                        className="flex items-center justify-between rounded border border-border/60 px-2.5 py-1.5"
                      >
                        <span className="text-2xs text-muted-foreground capitalize">
                          {stage.replace(/_/g, " ")}
                        </span>
                        <span className="font-sans tabular-nums font-semibold">{count}</span>
                      </div>
                    ),
                  )}
                </div>
                {report.clientHealth.atRiskCount > 0 && (
                  <div className="text-2xs text-destructive">
                    {report.clientHealth.atRiskCount} active client
                    {report.clientHealth.atRiskCount === 1 ? "" : "s"} at risk
                  </div>
                )}
              </div>
              <div className="rounded-xl border border-border bg-card p-4 space-y-2">
                ...
                <div className="text-2xs text-muted-foreground">
                  {report.hiringPipeline.newApplicantsThisWeek} new applicant
                  {report.hiringPipeline.newApplicantsThisWeek === 1 ? "" : "s"} this week
                </div>
              </div>
            </div>
          </div>
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.name} className="border-t border-border/70">
              ...
              {title === "Setters" && (
                <td className="p-2.5 text-right font-sans tabular-nums">{r.sets}</td>
              )}
              <td className="p-2.5 text-right font-sans tabular-nums">{r.closes}</td>
              <td className="p-2.5 text-right font-sans tabular-nums text-spectrum-hot">
                {fmtMoneyCents(r.cashCents)}
              </td>
            </tr>
          ))}
          {rows.length === 0 && (
            <tr>
              <td colSpan={title === "Setters" ? 4 : 3}>
                <EmptyState
                  icon={<FileText className="h-4 w-4" />}
                  title={`No ${title.toLowerCase()} activity this week`}
                  description="Nothing logged in the last 7 days."
                />
              </td>
            </tr>
          )}
