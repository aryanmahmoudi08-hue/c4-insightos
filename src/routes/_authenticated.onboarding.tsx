...
const DOUBLE_DOWN_KEYS = new Set([
  "first_touchpoint",
  "pivotal_moment",
  "beliefs_shifted",
  "content_type_helped",
]);
const signalFor = (k: string): Signal =>
  BOTTLENECK_KEYS.has(k) ? "bottleneck" : DOUBLE_DOWN_KEYS.has(k) ? "double_down" : null;
...
const signalBadge = (s: Signal) =>
  s === "bottleneck" ? (
    <span className="inline-flex items-center gap-1 rounded bg-destructive/10 text-destructive px-1.5 py-0.5 text-4xs font-semibold uppercase tracking-wider">
      <TrendingDown className="h-2.5 w-2.5" />
      Bottleneck
    </span>
  ) : s === "double_down" ? (
    <span className="inline-flex items-center gap-1 rounded bg-success/10 text-success px-1.5 py-0.5 text-4xs font-semibold uppercase tracking-wider">
...
const QUESTIONS: { key: string; q: string; type: QType; options?: string[]; section: Section }[] = [
  ...
  {
    section: "sales",
    key: "first_touchpoint",
    q: "What was the first piece of content / touchpoint that made you discover us?",
    type: "short",
  },
  {
    section: "sales",
    key: "pivotal_moment",
    q: "What was the moment you decided 'I have to work with them'?",
    type: "short",
  },
  {
    section: "sales",
    key: "objections_before",
    q: "What hesitations or doubts almost stopped you from joining?",
    type: "short",
  },
  {
    section: "sales",
    key: "join_sooner",
    q: "What could we have done to make you join 30 days sooner?",
    type: "long",
  },
  { section: "sales", key: "why_us", q: "Why did you choose us over competitors?", type: "long" },
  {
    section: "sales",
    key: "current_pain",
    q: "What is the #1 problem/frustration you're hoping we solve?",
    type: "short",
  },
  {
    section: "sales",
    key: "desired_identity",
    q: "12 months from now, who do you want to have become?",
    type: "short",
  },
  {
    section: "sales",
    key: "tried_before",
    q: "What have you tried before that didn't work, and why?",
    type: "long",
  },
  {
    section: "sales",
    key: "fear",
    q: "What's your biggest fear about this not working?",
    type: "long",
  },
  {
    section: "sales",
    key: "beliefs_shifted",
    q: "What belief [income wise] shifted when you saw our content?",
    type: "long",
  },
  {
    section: "sales",
    key: "content_type_helped",
    q: "Which content type helped most?",
    type: "choice",
    options: ["Reels", "Stories", "YouTube", "DMs", "Ads"],
  },
  {
    section: "sales",
  ...
  // --- Fulfillment intake (what the coach/CSM needs day 1) ---
  {
    section: "fulfillment",
    key: "current_revenue",
    q: "Current monthly revenue (or close estimate)",
    type: "short",
  },
  {
    section: "fulfillment",
    key: "target_revenue_90d",
    q: "Target monthly revenue 90 days from now",
    type: "short",
  },
  {
    section: "fulfillment",
    key: "north_star_outcome",
    q: "If we accomplish ONE thing together, what is it?",
    type: "long",
  },
  {
    section: "fulfillment",
    key: "success_metric",
    q: "How will you measure success at day 30 / 60 / 90?",
    type: "long",
  },
  {
    section: "fulfillment",
    key: "current_offer",
    q: "What are you selling right now? (offer, price, delivery)",
    type: "long",
  },
  {
    section: "fulfillment",
    key: "audience_size",
    q: "Audience size + main platform (IG followers / email list / etc.)",
    type: "short",
  },
  {
    section: "fulfillment",
    key: "current_funnel",
    q: "Walk us through your current funnel: traffic → lead → sale",
    type: "long",
  },
  {
    section: "fulfillment",
    key: "biggest_bottleneck",
    q: "What is the single biggest bottleneck stopping growth right now?",
    type: "long",
  },
  {
    section: "fulfillment",
    key: "team_setup",
    q: "Who is on your team today? (setter, closer, editor, VA, etc.)",
    type: "long",
  },
  {
    section: "fulfillment",
    key: "tech_stack",
    q: "Tools you use (CRM, scheduling, email, payments, automation)",
    type: "long",
  },
  {
    section: "fulfillment",
    key: "weekly_hours",
    q: "Hours per week you can commit to implementation",
    type: "choice",
  ...
  },
  {
    ...
    type: "choice",
    options: ["Watch & model", "Live coaching", "1:1 feedback", "Async written", "Do-it-with-me"],
  },
  {
    section: "fulfillment",
    key: "accountability_pref",
    q: "How do you want to be held accountable?",
    type: "choice",
    options: [
      "Weekly check-in",
      "Daily Slack",
  ...
  {
    section: "fulfillment",
    key: "blockers_personal",
    q: "Anything personal we should know that could affect pace? (health, family, travel, etc.)",
    type: "long",
  },
  {
    section: "fulfillment",
    key: "wins_so_far",
    q: "What's working right now that we should NOT change?",
    type: "long",
  },
  {
    section: "fulfillment",
    key: "non_negotiables",
    q: "Non-negotiables — things you will NOT do (channels, tactics, hours)",
    type: "long",
  },
  ...
  { section: "logistics", key: "best_contact", q: "Best way + time to reach you", type: "short" },
  {
    section: "logistics",
    key: "kickoff_date",
    q: "Earliest date you can start implementing this week",
    type: "short",
  },
];
...
const SECTION_META: Record<Section, { label: string; hint: string }> = {
  ...
  fulfillment: {
    ...
    hint: "What your coach / CSM needs to deliver outcomes",
  },
  logistics: { label: "Logistics", hint: "How and when to reach the mentee" },
};
...
function Onboarding() {
  ...
  const [insightsByResp, setInsightsByResp] = useState<
    Record<
      string,
      {
        bottlenecks: { title: string; body: string; recommendation: string }[];
        double_down: { title: string; body: string; recommendation: string }[];
      }
    >
  >({});
  ...
  const analyzeMut = useMutation({
    mutationFn: (id: string) =>
      devBypass ? withMockDelay(mockIntakeInsights()) : analyzeFn({ data: { responseId: id } }),
    onSuccess: (res, id) => {
      setInsightsByResp((m) => ({ ...m, [id]: res }));
  ...
  const aggregateMut = useMutation({
    mutationFn: () =>
      devBypass
        ? withMockDelay(mockIntakeInsights())
        : aggregateFn({ data: { from: activeRange.from, to: activeRange.to } }),
    onSuccess: (res) => {
      setAggInsights(res);
      toast.success(`Analyzed ${res.sampleSize ?? 0} intakes`);
    },
    onError: (e) => toast.error(e instanceof Error ? e.message : "Failed"),
  });
  ...
  const { data: responses } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, responses, mechanism_signals, submitted_at, created_at, share_token, clients(full_name, offer_name)",
        )
        .eq("org_id", orgId!)
  ...
  const create = useMutation({
    mutationFn: async (f: FormData) => {
      ...
      QUESTIONS.forEach((q) => {
        answers[q.key] = String(f.get(q.key) || "");
      });
    ...
    onSuccess: () => {
      toast.success("Onboarding captured · content team alerted");
  ...
  const updateResp = useMutation({
    mutationFn: async ({ id, answers }: { id: string; answers: Record<string, string> }) => {
      const { error } = await supabase
        .from("onboarding_responses")
        .update({ responses: answers, mechanism_signals: mechanismSignalsFor(answers) })
        .eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <>
      <TopBar
        title="Mentee Onboarding"
        subtitle="Deep intake — pain, beliefs, identity shifts feeding the content engine"
      />
      <div className="p-6 space-y-5">
          ...
          <BentoCell span="hero">
            <div className="hover-lift relative flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card p-5">
              ...
              <div className="relative mt-1 text-2xs text-muted-foreground">
                Every submission's "first touchpoint" / "decision to join" / "join sooner" answers
                are scored against the 4 content mechanisms and tagged here — feeds Content Signals'
                mix directly.
              </div>
        ...
        <div className="flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
            Each submission triggers a content-team event with extracted beliefs & pain
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Onboarding intake</DialogTitle>
              </DialogHeader>
              <form
                className="space-y-3"
                onSubmit={(e) => {
              ...
              >
                <div className="space-y-1.5">
                  ...
                  <Select name="client_id">
                    <SelectTrigger>
                      <SelectValue placeholder="Pick mentee" />
                    </SelectTrigger>
                    <SelectContent>
                      {(clients ?? []).map((c) => (
                ...
                <Button type="submit" className="w-full" disabled={create.isPending}>
                  {create.isPending ? "Saving…" : "Submit intake"}
                </Button>
              </form>
            </DialogContent>
          </Dialog>
        ...
        <div className="rounded-lg border-2 border-accent/40 bg-gradient-to-br from-accent/5 to-card p-4 space-y-3">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="min-w-0">
              <div className="flex items-center gap-2 text-sm font-semibold">
                <Sparkles className="h-4 w-4 text-accent" /> Pattern analysis · ALL mentees
              </div>
              <div className="text-2xs text-muted-foreground">
                Aggregates answers from{" "}
                <span className="font-semibold text-foreground">every intake</span> in the selected
                range. Finds repeating bottlenecks to fix and winning patterns to amplify across
                your mentee base.
              </div>
            </div>
            <Button
          ...
          {aggInsights &&
            (aggInsights.sampleSize === 0 ? (
              <div className="text-xs text-muted-foreground italic animate-in fade-in-0 duration-300">
              ...
              <div className="grid md:grid-cols-2 gap-3 animate-in fade-in-0 slide-in-from-top-1 duration-300">
                <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 space-y-2">
                  <div className="flex items-center gap-1.5 text-2xs font-semibold uppercase tracking-wider text-destructive">
                    <TrendingDown className="h-3 w-3" /> Bottlenecks across {aggInsights.sampleSize}{" "}
                    intakes
                  </div>
                  {aggInsights.bottlenecks.length === 0 && (
                    <div className="text-xs text-muted-foreground italic">None surfaced.</div>
                  )}
                ...
                <div className="rounded-md border border-success/30 bg-success/5 p-3 space-y-2">
                  <div className="flex items-center gap-1.5 text-2xs font-semibold uppercase tracking-wider text-success">
                    <TrendingUp className="h-3 w-3" /> Double down across {aggInsights.sampleSize}{" "}
                    intakes
                  </div>
                  {aggInsights.double_down.length === 0 && (
                    <div className="text-xs text-muted-foreground italic">None surfaced.</div>
                  )}
                  {aggInsights.double_down.map((i, idx) => (
                    <div key={idx} className="space-y-1">
                      ...
                      <div className="text-2xs text-success">
                        <span className="font-semibold">Amplify: </span>
                        {i.recommendation}
                      </div>
                    </div>
                  ))}
          ...
          <TabsContent value="intakes">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="rounded-lg border border-border bg-card overflow-hidden lg:col-span-1">
                <div className="bg-muted/40 px-3 py-2 text-2xs uppercase tracking-wider text-muted-foreground">
                  Recent intakes
                </div>
                <div className="p-2 border-b border-border">
                  <div className="relative">
                    ...
                    <Input
                      placeholder="Search by mentee or answers…"
                      value={query}
                ...
                <div className="divide-y divide-border max-h-[60vh] overflow-y-auto">
                  {(responses ?? [])
                    .filter((r) => {
                      ...
                      const ans = Object.values((r.responses ?? {}) as Record<string, string>)
                        .join(" ")
                        .toLowerCase();
                    ...
                    .map((r) => (
                      <button
                        ...
                        className={`w-full text-left p-3 hover:bg-muted/30 ${selected === r.id ? "bg-muted/40" : ""}`}
                      >
                        <div className="font-medium text-sm">
                          {r.clients?.full_name ?? "(no mentee linked)"}
                        </div>
                        <div className="text-2xs text-muted-foreground">
              ...
              <div className="rounded-lg border border-border bg-card p-4 lg:col-span-2">
                {selected ? (
                          ...
                          {editMode ? (
                            <div className="flex gap-1">
                              ...
                              <Button
                                ...
                                onClick={() => updateResp.mutate({ id: r.id, answers: editDraft })}
                              >
                        ...
                        <div className="rounded-lg border border-border bg-muted/20 p-3 space-y-3">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                              <Sparkles className="h-3.5 w-3.5 text-accent" /> AI insights from this
                              intake
                            </div>
                            <Button
                              size="sm"
                            ...
                            >
                              {analyzeMut.isPending && analyzeMut.variables === r.id ? (
                                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                              ) : (
                                <Wand2 className="h-3.5 w-3.5" />
                              )}
                          ...
                          {insightsByResp[r.id] ? (
                            <div className="grid md:grid-cols-2 gap-3">
                              <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 space-y-2">
                                ...
                                {insightsByResp[r.id].bottlenecks.length === 0 && (
                              ...
                              <div className="rounded-md border border-success/30 bg-success/5 p-3 space-y-2">
                                ...
                                {insightsByResp[r.id].double_down.length === 0 && (
                                  <div className="text-xs text-muted-foreground italic">
                                    None surfaced.
                                  </div>
                                )}
                            ...
                            <div className="text-xs text-muted-foreground">
                              Click <span className="font-semibold">Analyze</span> to extract
                              fixable bottlenecks (red questions) and high-leverage patterns to
                              double down on (green questions) from this mentee's answers.
                            </div>
                          )}
                        </div>
                                  ...
                                  {editMode ? (
                                    q.type === "long" ? (
                                      <Textarea
                                        ...
                                        value={draft[q.key] ?? ""}
                                        onChange={(e) =>
                                          setEditDraft((d) => ({ ...d, [q.key]: e.target.value }))
                                        }
                                      />
                                    ) : q.type === "choice" ? (
                                      <Select
                                    ...
                                    <div
                                      className={`rounded p-3 text-sm whitespace-pre-wrap ${sig === "bottleneck" ? "bg-destructive/5" : sig === "double_down" ? "bg-success/5" : "bg-muted/30"}`}
                                    >
                                      {ans[q.key]}
                                    </div>
                                  ) : (
                  ...
                  <div className="text-center text-xs text-muted-foreground py-10">
                    Select an intake to view answers
                  </div>
                )}
              </div>
            </div>
                  ...
                  <div key={group.label} className="rounded-lg border border-border bg-card p-4">
                    ...
                    {phrases.length === 0 ? (
                      <div className="text-xs text-muted-foreground py-6 text-center">
                        Need 2+ intakes with repeated phrases to surface themes.
                      </div>
                    ) : (
...
const THEME_GROUPS = [
  {
    label: "Pain points",
    keys: ["current_pain", "tried_before"],
    icon: MessageSquareQuote,
    hint: "What problems mentees say they're trying to solve — feeds hook + offer copy",
  },
  {
    label: "Pivotal moments",
    keys: ["pivotal_moment", "beliefs_shifted"],
    icon: Sparkles,
    hint: "The exact moments + beliefs that converted them — replicate in content",
  },
  {
    label: "Objections that almost killed the sale",
    keys: ["objections_before", "fear"],
    icon: Brain,
    hint: "Pre-emptive content + script targets",
  },
  {
    label: "Desired identity",
    keys: ["desired_identity", "why_us"],
    icon: Cloud,
    hint: "Aspirational language to mirror in long-form + sales",
  },
] as const;
