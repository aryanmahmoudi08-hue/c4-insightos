...
| **Meta / Instagram Ads** | **OAuth built** (`87681e3`). Confirmed 2026-09-24 that the business runs paid Meta ads. Needs a Meta app: create one, set `META_APP_ID`/`META_APP_SECRET` as server env vars, and register `{origin}/api/public/oauth/meta` as a redirect URI in its Facebook Login settings. Then Settings → Connections → Connect with Meta. **Still to build after that:** the actual spend pull into `acquisition_spend` (the destination table already exists and is idempotent) | **D** — needs a Meta app, then one more build step |
...
| **Connector-registry OAuth capability** | **Done** (`87681e3`) — was the cross-cutting blocker for every ad platform. TikTok/YouTube/LinkedIn can now reuse the same state/token machinery; only their provider-specific endpoints and scopes differ | **A** |
